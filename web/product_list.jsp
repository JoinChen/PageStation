...
<%--分页显示的开始 --%>
    	<div style="text-align:center">
    		...
    		<a href="${pageContext.request.contextPath}/PageServlet?num=1">首页</a>
    		
    		<c:if test="${page.currentPageNum != 1}">
    			<a href="${pageContext.request.contextPath}/PageServlet?num=${page.prePageNum}">上一页</a>
    		</c:if>
    		...
    		<c:forEach begin="${page.startPage}" end="${page.endPage}" var="pagenum">
    		   ...
    		   <c:if test="${page.currentPageNum!=pagenum}">
    		   		<a href="${pageContext.request.contextPath}/PageServlet?num=${pagenum}">${pagenum}</a>
    		   </c:if>
    		</c:forEach>
    		
    		<c:if test="${page.currentPageNum != page.totalPageNum}">
    			<a href="${pageContext.request.contextPath}/PageServlet?num=${page.nextPageNum}">下一页</a>
    		</c:if>
    		
    		
    		<a href="${pageContext.request.contextPath}/PageServlet?num=${page.totalPageNum}">末页</a>
    		...
    		<script type="text/javascript">
    			function jump(){
    				...
    				//转向分页显示的Servlet
    				window.location.href="${pageContext.request.contextPath}/PageServlet?num="+pagenum;
    			}
    		</script>
    	</div>
    	<%--分页显示的结束--%>
